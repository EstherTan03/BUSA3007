...
// ---- Minimal ERC20 ABI for approve/allowance/decimals (ERC-20 path) ----
const ERC20_ABI = [
...
function PropertyCard({ property = {}, currentWallet: walletFromProp }) {
  ...
  // ---- Smarter deposit that supports payable and ERC20 flows ----
    ...
    if (typeof escrow.paymentToken === 'function') {
      try {
        ...
        if (paymentTokenAddr && paymentTokenAddr !== ethers.ZeroAddress) {
          const erc20 = new ethers.Contract(paymentTokenAddr, ERC20_ABI, signer);

          // NOTE: assuming 18 decimals; if your token uses other decimals, ideally compute from on-chain price.
          let amount = valueWei;
          try {
            const dec = await erc20.decimals();
            // If not 18, you may want to scale amount accordingly (depends on contract’s stored price).
            // For now we keep valueWei as-is.
          } catch {}

          const current = await erc20.allowance(buyerAddr, escrowAddress).catch(() => 0n);
          if (current < amount) {
            const txA = await erc20.approve(escrowAddress, amount);
            await txA.wait();
          }
          ...
          for (const [name, args] of erc20Candidates) {
            const txB = await callFn(name, args, {}); // no value for ERC20 path
            if (txB) return txB;
          }
  ...
  const buyNow = async () => {
    try {
      ...
      try {
        ...
        setTxMsg(
          '✅ Payment deposited to escrow. (Buyer confirm step not available or not required.)'
        );
      }
    } catch (e) {
      // eslint-disable-next-line no-console
